...
Ikuti langkah-langkah berikut untuk menjalankan project di tempat Kalian:

Pastikan layanan MySQL Kalian sudah aktif (misalnya melalui XAMPP atau MySQL Service). 
...
Aplikasi akan berjalan secara otomatis. Lalu Kalian dapat mencoba seluruh endpoint melalui Swagger UI yang tersedia di:
```bash 
...
Dibawah ini adalah contoh gambar swagger yang Kalian buka menggunakan link port localhost Kalian
...
Proyek ini menggunakan pendekatan Code-First dengan Entity Framework Core. Kalian tidak perlu mengimpor file SQL secara manual karena skema tabel akan dibuat secara otomatis melalui fitur Migration. 


Konfigurasi Koneksi: Buka file appsettings.json dan sesuaikan ConnectionStrings dengan kredensial MySQL lokal Kalian. 
```bash
"ConnectionStrings": {
...
``` 
Setelah proses selesai, database siap digunakan untuk menyimpan data Todo Kalian.

- Fitur & Validasi
- Async/Await: Seluruh endpoint diimplementasikan secara asinkron untuk performa optimal. 
- HTTP Status Codes: Menggunakan response code stKalianr industri seperti 200 OK, 201 Created, dan 404 Not Found. 
